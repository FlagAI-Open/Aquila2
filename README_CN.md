...
<br>
<p>
    注：发现在预训练任务数据集中存在GSM8K测试数据泄露问题。
彻查分析后发现，数据泄露发生于某多次合作的外部团队所推荐的数学数据集A（超过2百万样本），其包含未经过处理的GSM8K测试集（1319样本）。在与该外部团队成员沟通中，对方建议该数据集A全部使用。团队只进行了常规的去重和质量检测，未就是否混入GSM8K测试数据进行过滤检查，就将该数据集A放入了预训练数据切片中，实为工作中的疏漏。

目前，我们已经从评测结果中删除GSM8K的评估结果。

团队一直严格遵循训练数据不能包含测试数据的工作原则。吸取本次由于对外部数据来源没有进行清查而造成失误的教训，我们正在对2万亿token 的全量数据进行各种测试数据集的排查，目前MMLU、CMMLU等测试数据并未存在预训练数据中，所有20多个测试数据集的排查全部完成还需要一定时间，会第一时间更新排查结果。对未来，团队将持续对榜单中的其他数据集进行自查，以确保数据完整性，并将持续关注此问题，及时更新检查结果。

团队始终对数据隐私和安全性保持严谨态度，并积极处理数据泄露问题，以确保后续工作的公正和严谨。
