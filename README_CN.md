...
## 公告
在Aquila预训练任务数据集中，存在一个问题<sup>[[1]](https://arxiv.org/abs/2310.19341)</sup>，即包含在8月25日至9月3日训练数据切片中的数据泄露。经分析，数据泄露发生于某外部团队推荐的数学数据集A（>2百万样本），其包含未经过处理的GSM8K测试集（1319样本）。在与该外部团队成员沟通中，他们明确指出该数据集A已用于微调实验并获得显著的数学能力提升效果，并计划近期发布。由于对该开源数据集的信任，Aquila团队只进行了简单的去重和质量检测，就将其放入了预训练数据切片中。

Aquila团队采取以下处理措施：
- 从评测结果中删除GSM8K的评估结果。
- 团队已分别对MMLU和CMMLU进行了全面的自查，未发现其他问题。
- 未来，团队将持续对榜单中的其他数据集进行自查，以确保数据完整性，并将持续关注此问题，及时更新检查结果。

强调：Aquila2 GSM8K上性能主要来自Flan<sup>[[2]](https://arxiv.org/pdf/2301.13688.pdf)</sup>和自建数据集。Aquila团队始终对数据隐私和安全性保持严谨态度，并积极处理数据泄露问题，以确保后续工作的公正和严谨。
