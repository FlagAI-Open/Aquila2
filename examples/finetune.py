...
def preprocess(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        if convo_template == 'aquila-cpt-v1':
            ## <|endofpiece|>
            value_to_select = 100001
            torch.set_printoptions(threshold=np.inf)
            indices_to_remove = (input_id == value_to_select).nonzero()
            mask = torch.ones_like(input_id, dtype=torch.bool)
            mask[indices_to_remove] = False
            input_id = torch.masked_select(input_id, mask)
            target = torch.masked_select(target, mask)
            import torch.nn.functional as F
            padding_length = tokenizer.model_max_length - input_id.size(0)
            F.pad(input_id, (0, padding_length), value=tokenizer.pad_token_id)
            F.pad(target, (0, padding_length), value=IGNORE_TOKEN_ID)
        elif convo_template == 'aquila-cpt-v2':
            ## <|endofpiece|>
            value_to_select = 100001
            torch.set_printoptions(threshold=np.inf)
            indices_to_remove = (input_id == value_to_select).nonzero()
