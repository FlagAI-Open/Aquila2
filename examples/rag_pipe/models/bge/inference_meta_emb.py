...
class EmbDataset(Dataset):
    ...
    def __getitem__(self, item):
        # combine all info
        texts = (
            f"title: {self.data[item].get('title','')}"
            + f"\nauthors: {self.data[item].get('authors', '')}"
            + f"\nabstract: {self.data[item].get('abstract','')}"
            + f"\ndetails: {self.data[item].get('details','')}"
        )

        # texts = f"content: {self.data[item]['content']}" + f"cellname: {self.data[item]['cell_name']}"
        # print(texts)
...
def main(args):
    ...
    print(f"total sample num is : {len(loader)}, ")
    model.eval()
    existing_data = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-path", type=str, default="meta.npy")
