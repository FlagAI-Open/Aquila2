...
class QuerySummarizer:
    def __init__(self):
        prompt_template = (
            """Summarize the topic of the text.\nQuestion: {question}\nSummary:"""
        )
        llm = ChatOpenAI(temperature=0, model_name="gpt-3.5-turbo")
        prompt = PromptTemplate(
...
class SearchIndex:
    def __init__(
        ...
        # create meta_bm25_searcher and meta_index_reader
        ...
        self.tokenizer = retrial_tokenizer
        # self.model = AutoModel.from_pretrained(BGE_RETRIVAL)
        # self.model = self.model.to(self.device)
        self.model = retrial_model
        ...
        if rerank == "enable":
            if retrieval_type != "merge":
                if retrieval_type == "semantic":
                    ...
                    query_embedding = (
                        torch.nn.functional.normalize(query_embedding, p=2, dim=1)
                        .detach()
                        .cpu()
                        .numpy()
                    )
                    ...
                    distances, ids = [int(e.score) for e in hits], [
                        int(e.docid) for e in hits
                    ]
                pairs = []
                for i in ids:
            ...
            if retrieval_type != "merge":
                ...
                hits = self.abstract_bm25_searcher.search(query, num)
                for d, i in zip(
                    [int(e.score) for e in hits], [int(e.docid) for e in hits]
                ):
                ...
                for k in original_dict.keys():
                    if original_dict[k].get("original semantic score") is None:
                        original_dict[k]["original semantic score"] = (
                            query_embedding @ self.abstract_doc_emb[k].T
                        )[0]
                    if original_dict[k].get("original term frequency score") is None:
                        original_dict[k][
                            "original term frequency score"
                        ] = self.abstract_index_reader.compute_bm25_term_weight(
                            str(k), query, analyzer=None
                        )
                    original_dict[k]["merge score"] = float(
                        original_dict[k]["original term frequency score"]
                    ) * 0.2 + float(original_dict[k]["original semantic score"])
                sorted_data = dict(
                    sorted(
                        original_dict.items(),
                        key=lambda x: x[1]["merge score"],
                        reverse=True,
                    )
                )
                result_dict = {}
                ...
                for i in range(num):
                    result_dict[str(len(result_dict.keys()))] = {
                        "original semantic score": str(
                            sorted_data[all_keys[i]]["original semantic score"]
                        ),
                        "original term frequency score": str(
        ...
        if rerank == "enable":
                ...
                for i in ids:
                    pairs.append(
                        [
                            ...
                            + f"\nauthors: {self.data[i]['authors']}"
                            + f"\nabstract: {self.data[i]['abstract']}"
                            + f"\ndetails: {self.data[i].get('details','')}",
                        ]
                    )
                with torch.no_grad():
                    all_scores = []
            ...
            if retrieval_type != "merge":
                ...
                for k in original_dict.keys():
                    if original_dict[k].get("original semantic score") is None:
                        original_dict[k]["original semantic score"] = (
                            query_embedding @ self.meta_doc_emb[k].T
                        )[0]
                    if original_dict[k].get("original term frequency score") is None:
                        original_dict[k][
                            "original term frequency score"
                        ] = self.meta_index_reader.compute_bm25_term_weight(
                            str(k), query, analyzer=None
                        )
                    original_dict[k]["merge score"] = float(
                        original_dict[k]["original term frequency score"]
...
class SearchTool:
    ...
    def conversation_search(
        ...
        # self.log.flush()
        if target == "original":
            return self.search_index.search_original(
                query, retrieval_type, num, rerank, rerank_num
            )
        else:
            return self.search_index.search_conditional(
                query, retrieval_type, num, rerank, rerank_num
            )
    ...
    def search(
        ...
        if search_type == "by conversation":
            return self.conversation_search(
                query, retrieval_type, target, num, rerank, rerank_num
            )
